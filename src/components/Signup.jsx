...
const Field = ({ label, error, hint, children }) => (
  <div className="flex flex-col gap-1">
    ...
    {children}
    {error ? (
      <span className="flex items-center gap-1 text-xs text-red-500"><span>⚠</span> {error}</span>
    ) : hint ? (
...
const ROLES = [
  {
    value: "customer",
    label: "Customer",
    hint: "Book repair services",
    backendRole: "customer",
  },
  {
    value: "provider",
    label: "Mechanic",
    hint: "Offer repair services",
...
const SignupPage = () => {
  ...
  // role: "customer" | "provider"  — maps directly to what backend expects
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const userId = user?.id || user?.userId || user?._id || res.data?.id || res.data?.userId || null;
      const email  = user?.email || res.data?.email || form.email;

      if (token)  localStorage.setItem("token",  token);
      if (email)  localStorage.setItem("email",  email);
      else { toast.error("Signup response missing email."); return; }
      if (userId) localStorage.setItem("userId", String(userId));
      else { toast.error("Signup response missing user ID."); return; }

      // Store role so VerifyEmail knows where to redirect after OTP
      // customer → /login   |   provider → /booking/1
      localStorage.setItem("role", role);

      toast.success("Account created! Please verify your email.", { autoClose: 1800 });
  ...
  const inputClass = (name) =>
    `w-full px-3 py-2.5 border rounded-lg text-sm text-gray-900 bg-white outline-none transition-all duration-200
     focus:border-blue-700 focus:ring-2 focus:ring-blue-700/10
     ${touched[name] && errors[name] ? "border-red-300 bg-red-50/50" : "border-gray-300"}`;
      ...
      <div className="flex items-center justify-center min-h-screen p-4 bg-gray-100">
        <div className="flex flex-col md:flex-row w-full max-w-[1100px] bg-white rounded-xl overflow-hidden shadow-[0_10px_40px_rgba(0,0,0,0.10)]">
          ...
          <div className="flex flex-col flex-1 max-h-screen p-6 overflow-y-auto md:p-10">
            ...
            {/* ── Role Tab Switcher ── */}
            <div className="flex p-1 mb-6 bg-gray-100 rounded-lg gap-1">
              {ROLES.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-md
                ...
                <Field label="Password" error={touched.password && errors.password}>
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowPw(!showPw)}
                      className="absolute text-gray-400 -translate-y-1/2 right-3 top-1/2 hover:text-gray-600 transition-colors">
