  ...
  toggle: (active) => (
    <svg viewBox="0 0 44 24" fill="none" className="w-9 h-5">
      <rect x="0" y="0" width="44" height="24" rx="12"
        fill={active ? "#1C52AF" : "#D1D5DB"} />
      <circle
        cx={active ? "32" : "12"} cy="12" r="9"
        fill="white"
...
/**
 * Derive a display group ("Today" / "Yesterday" / "Earlier") from a timestamp.
 */
...
function mapNotification(raw) {
  return {
    id: raw.id,
    group: getGroup(raw.createdAt ?? raw.created_at ?? Date.now()),
    title: raw.title ?? "Notification",
    message: raw.message ?? raw.body ?? "",
    time: relativeTime(raw.createdAt ?? raw.created_at ?? Date.now()),
    timestamp: new Date(raw.createdAt ?? raw.created_at ?? Date.now()).getTime(),
    type: raw.type ?? "update",
    unread: raw.isRead === false || raw.read === false || !raw.readAt,
    meta: raw.meta ?? {
      vehicle: raw.vehicle ?? null,
      booking: raw.bookingId ?? raw.booking ?? null,
...
const Toast = ({ message, type = "success", onDone }) => {
  ...
  return (
    ...
    >
      <span className={type === "error" ? "text-red-200" : "text-emerald-400"}>{Icons.check}</span>
      {message}
    </motion.div>
  );
};
...
export default function Notification() {
  ...
  const fetchPage = useCallback(async (page = 1, append = false) => {
    try {
      ...
      const { data: items = [], meta: m = {} } = res.data;
  ...
  // Re-fetch when tab or type filter changes
  ...
  const toggleNotifications = useCallback(async () => {
    ...
    try {
      await api.post(`/notifications/${next ? "enable" : "disable"}`, {
        deviceType: "WEB",
        token: "web_" + (localStorage.getItem("deviceToken") || Math.random().toString(36).slice(2)),
      });
      setNotifEnabled(next);
      showToast(`Notifications ${next ? "enabled" : "disabled"}`);
    } catch {
      showToast("Couldn't update notification settings", "error");
    } finally {
      setTogglingNotif(false);
    }
  }, [notifEnabled, togglingNotif]);
          ...
          >
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 opacity-90 hover:opacity-100 transition-opacity disabled:opacity-50"
              >
                {Icons.toggle(notifEnabled)}
              </button>
