...
function CancelModal({ open, onClose, jobId, onSuccess }) {
  ...
  const doCancel = async () => {
    ...
    try {
      ...
      // The backend can throw a 500 AFTER it has already applied the
      // cancellation (e.g. a notification/email step fails post-write).
      // Don't trust the HTTP status alone — verify the real job status
      ...
      try {
        ...
        // verification call itself failed — fall through to showing the error
      }
      setError(err.response?.data?.message || "Cancellation failed. Please try again.");
    } finally {
      setLoading(false);
...
// ─── Tab Button (redesigned: underline style, lives in the header) ───
function TabButton({ tab, activeTab, onClick, count }) {
  const isActive = activeTab === tab.key;
  return (
    <button
      onClick={onClick}
      className="relative shrink-0 flex items-center gap-1.5 px-1 py-3 text-sm font-medium transition-colors"
    >
      <span className={isActive ? "text-gray-900" : "text-gray-400 hover:text-gray-600"}>
        {tab.label}
      </span>
      {count > 0 && (
        <span
          className={`text-[11px] font-semibold px-1.5 py-0.5 rounded-md leading-none transition-colors ${
            isActive ? "bg-blue-50 text-[#1C52AF]" : "bg-gray-100 text-gray-400"
          }`}
        >
          {count}
        </span>
      ...
      <span
        className="absolute -bottom-px left-0 right-0 h-[2px] rounded-full bg-[#1C52AF] transition-transform duration-300 origin-left"
...
const ActiveServices = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      ...
      {/* Header + Tabs combined into one sticky bar */}
      <div className="bg-white/80 backdrop-blur-sm sticky top-0 z-30 border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-4 sm:pt-5">
            <div className="flex-1 min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900 tracking-tight">My Services</h1>
              <p className="text-xs sm:text-sm text-gray-500 mt-0.5 sm:mt-1">Track and manage your vehicle maintenance bookings</p>
            </div>
          ...
          {/* Underline Tabs */}
          <div className="flex gap-6 overflow-x-auto -mb-px mt-1">
            {TABS.map((tab) => (
              <TabButton key={tab.key} tab={tab} activeTab={activeTab} onClick={() => switchTab(tab.key)} count={countFor(tab)} />
            ))}
          </div>
