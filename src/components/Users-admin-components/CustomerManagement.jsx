...
//       {/* COMPACT STATS CARDS - CONNECTED TO API */}
//       <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6">
//         {/* Total Customers */}
//         <div className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
//           <div className="flex items-center gap-3">
...
//               ) : (
//                 <h1 className="text-lg font-bold text-gray-800">{metrics.totalUsers.toLocaleString()}</h1>
//               )}
//             </div>
//           </div>
//           <p className="text-xs text-green-600 mt-1 flex items-center gap-1"><FaArrowUp className="w-3 h-3" />+12% this month</p>
...
//                 <div className="h-6 w-16 bg-gray-200 rounded animate-pulse mt-1"></div>
//               ) : (
//                 <h1 className="text-lg font-bold text-gray-800">{metrics.activeBookings.toLocaleString()}</h1>
//               )}
//             </div>
//           </div>
//           <p className="text-xs text-blue-600 mt-1 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse"></span>Live updates</p>
...
//         {/* New Users This Month (retention metric replaced) */}
//         <div className="p-3 bg-white rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
//           <div className="flex items-center gap-3">
//             <div className="p-2 bg-purple-50 text-purple-500 rounded-lg"><FaChartLine className="w-4 h-4" /></div>
//             <div>
...
//                 <div className="h-6 w-16 bg-gray-200 rounded animate-pulse mt-1"></div>
//               ) : (
//                 <h1 className="text-lg font-bold text-gray-800">{metrics.newUsersThisMonth.toLocaleString()}</h1>
//               )}
//             </div>
//           </div>
...
const CustomerManagement = () => {
  ...
  const fetchUsers = useCallback(async (page = 1, limit = 10) => {
    try {
      ...
      const response = await api.get(`/admin/users?limit=${limit}&page=${page}`);
      if (response.data) {
  ...
  // Client-side filtering on the current page data
  const filteredData = useMemo(() => {
    return users
      .filter(
        (item) =>
  ...
  }, [search, statusFilter, sort, users]);
  ...
  const handleEditUser = async (userData) => {
    try {
      await api.patch(`/admin/users/${currentUser.id}`, userData);
      setModalOpen(false);
      showToast("User updated successfully", "success");
      fetchUsers(currentPage, itemsPerPage);
    } catch (error) {
    ...
    <div className="bg-gray-50 min-h-screen font-sans">
              ...
              {modalMode === "view" && currentUser ? (
                <div className="space-y-4">
                  <div className="flex items-center gap-4 pb-4 border-b border-gray-100">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-gray-800">{currentUser.firstName} {currentUser.lastName}</h4>
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(currentUser.status)}`}>{currentUser.status}</span>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400 uppercase tracking-wider">Phone</p>
                      <div className="flex items-center gap-2 text-sm text-gray-700"><FaPhone className="text-gray-400 w-4 h-4" />{currentUser.phoneNumber || "N/A"}</div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400 uppercase tracking-wider">Role</p>
                      <div className="flex items-center gap-2 text-sm text-gray-700"><span className="capitalize">{currentUser.role}</span></div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400 uppercase tracking-wider">Joined</p>
                      <div className="flex items-center gap-2 text-sm text-gray-700"><FaCalendarAlt className="text-gray-400 w-4 h-4" />{formatDate(currentUser.createdAt)}</div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400 uppercase tracking-wider">Status</p>
                      <div className="flex items-center gap-2 text-sm text-gray-700"><span className={`w-2 h-2 rounded-full ${getStatusDot(currentUser.status)}`}></span>{currentUser.status}</div>
                    </div>
                    <div className="space-y-1">
                      <p className="text-xs text-gray-400 uppercase tracking-wider">Verified</p>
                      <div className="flex items-center gap-2 text-sm text-gray-700">{currentUser.isVerified ? <FaCheck className="text-emerald-500" /> : <FaTimes className="text-red-400" />}{currentUser.isVerified ? "Yes" : "No"}</div>
                    </div>
                  </div>
                </div>
              ) : (
                <form onSubmit={(e) => { e.preventDefault(); const formData = new FormData(e.target); const userData = { firstName: formData.get("firstName"), lastName: formData.get("lastName"), email: formData.get("email"), phoneNumber: formData.get("phoneNumber"), status: formData.get("status"), role: formData.get("role"), isActive: formData.get("isActive") === "true" }; modalMode === "add" ? handleAddUser(userData) : handleEditUser(userData); }} className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">First Name</label>
                      <input name="firstName" defaultValue={currentUser?.firstName || ""} required className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm" placeholder="First name" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Last Name</label>
                      <input name="lastName" defaultValue={currentUser?.lastName || ""} required className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm" placeholder="Last name" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Phone</label>
                      <input name="phoneNumber" defaultValue={currentUser?.phoneNumber || ""} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm" placeholder="+1 234 567 890" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Role</label>
                      <select name="role" defaultValue={currentUser?.role || "customer"} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm bg-white">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
                      <select name="status" defaultValue={currentUser?.status || "pending"} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm bg-white">
      ...
      <div className="flex flex-col gap-3 mb-6 md:flex-row md:items-center md:justify-between">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100">
            <FaSearch className="text-gray-400 w-4 h-4" />
            <input type="text" placeholder="Search users..." value={search} onChange={(e) => { setSearch(e.target.value); }} className="outline-none text-sm w-48" />
            {search && <button onClick={() => setSearch("")} className="text-gray-400 hover:text-gray-600"><FaTimes className="w-3 h-3" /></button>}
          </div>
          <div className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition">
            <FaFilter className="text-gray-400 w-4 h-4" />
            <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); }} className="outline-none text-sm bg-transparent">
              <option value="All">All Status</option>
              <option value="pending">Pending</option>
              <option value="active">Active</option>
              <option value="inactive">Inactive</option>
        ...
        <div className="flex items-center gap-4">
          <p className="text-sm text-gray-500">Showing <span className="font-semibold text-gray-700">{filteredData.length}</span> of {meta.totalItems} users</p>
          <select value={itemsPerPage} onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }} className="text-sm border border-gray-200 rounded-lg px-2 py-1 outline-none bg-white">
            <option value={10}>10 / page</option>
            <option value={20}>20 / page</option>
      ...
      <div className="overflow-hidden bg-white border border-gray-200 rounded-xl shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody>
            {usersLoading ? (
              ...
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i} className="border-t border-gray-100">
                  <td className="px-5 py-4"><div className="flex items-center gap-3"><div className="w-9 h-9 bg-gray-200 rounded-full animate-pulse"></div><div className="h-4 w-24 bg-gray-200 rounded animate-pulse"></div></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-32 bg-gray-200 rounded animate-pulse"></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-24 bg-gray-200 rounded animate-pulse"></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-16 bg-gray-200 rounded animate-pulse"></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-16 bg-gray-200 rounded animate-pulse"></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-20 bg-gray-200 rounded animate-pulse"></div></td>
                  <td className="px-5 py-4"><div className="h-4 w-24 bg-gray-200 rounded animate-pulse"></div></td>
                </tr>
              ))
            ) : (
              filteredData.map((user) => (
                <tr key={user.id} className="border-t border-gray-100 hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-gray-600">{user.email}</td>
                  <td className="px-5 py-4 text-gray-600">{user.phoneNumber || "N/A"}</td>
                  <td className="px-5 py-4">
                    <span className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full capitalize ${user.role === "provider" ? "bg-purple-50 text-purple-700 border border-purple-200" : "bg-blue-50 text-blue-700 border border-blue-200"}`}>
                      {user.role}
                    </span>
                  </td>
                  <td className="px-5 py-4">
            ...
            {!usersLoading && filteredData.length === 0 && (
              <tr>
        ...
        {/* Pagination */}
        {!usersLoading && meta.totalItems > 0 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50/50">
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => setCurrentPage((p) => Math.min(meta.totalPages, p + 1))} disabled={currentPage === meta.totalPages} className="p-2 text-gray-600 hover:bg-white hover:shadow-sm rounded-lg transition disabled:opacity-40 disabled:cursor-not-allowed"><FaChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        )}
      </div>
